...
- Default Brease API base URL now includes the `/v1` path segment
  (`https://api.brease.io/v1`). Apps relying on the built-in default no longer
  need to set `BREASE_BASE_URL` to reach the versioned API.

## [0.2.2] - 2026-05-13

### Added

- CLI now prompts for a template variant (`starter` or `base`) and pulls the
  matching subdir from `designatives/brease-next-templates`. `starter` ships
  the opinionated full scaffold; `base` ships bare CMS wiring.
